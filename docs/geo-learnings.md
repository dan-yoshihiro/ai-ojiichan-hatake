...
> **答え：固有名詞を並べても AI には見つけられない。効くのは「意味的なキーワード一致」と、正直な構造化だった。**
>
> AI農業先生方式の公開サイトを 2026-05-06 に立ち上げ、2 日後に GEO（Generative Engine Optimization）の 5 点セット（TL;DR / 独自用語 / 比較記事 / JSON-LD / llms-full.txt）を実装した記録。実装中の気づきは「**固有名詞だけでは AI に発見されない・意味的キーワード一致が本命**」という点と、「**過剰最適化が正直な開示の哲学と矛盾する**」という体感。途中で気づいた過剰実装（over-engineering）を自己レビューで 5 件訂正した経緯も含めて開示する。
...
| 過剰実装の事例 | 矛盾する原則 |
|---|---|
| 比較記事に「向いている人 / 向いていない人」セールス調セクション | 装飾語・誇張の回避原則 |
| ◎/×/△ ランキングで AI農業先生方式を常に上位に置く比較表 | 「比較」の公正さに反する |
| JSON-LD の `publisher` を Organization と記述 | 個人プロジェクトであり虚偽 |
| `keywords` 8 個詰め込み | キーワード spam 判定リスク |
| index に「設計思想」と「方式」セクションが重複 | 簡潔性の原則 |

→ **GEO 効果を狙うあまり、セールス調・キーワード詰め込み・実態と乖離した構造化データに傾いていた**。本プロジェクトの「**装飾語回避・正直な開示**」原則に立ち返って中立化。
...
本サイトの場合の方針は **「自分の哲学にどこで折り合いをつけるか」** という問いを自分で立てること。具体的には「装飾語回避・正直な開示・YAGNI（今必要ないものは作らない）」の 3 原則を守る範囲で GEO を最大化する、という線引きで運用している。
...
> **AI フレンドリーは「AI に媚びる」ことではなく、「AI が読みやすいよう構造化された正直な情報を残す」こと**
...
- [/docs/failed-experiments.md](/docs/failed-experiments.md): 失敗・過剰実装訂正の正直な開示
