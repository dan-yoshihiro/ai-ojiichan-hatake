...
> **TL;DR**: AI農業先生方式の公開サイトを 2026-05-06 に立ち上げ、2 日後に GEO（Generative Engine Optimization）の 5 点セット（TL;DR / 独自用語 / 比較記事 / JSON-LD / llms-full.txt）を実装した記録。実装中の気づきは「**固有名詞だけでは AI に発見されない・意味的キーワード一致が本命**」という点と、「**過剰最適化が正直な開示の哲学と矛盾する**」という体感。途中で気づいた過剰実装（over-engineering）を自己レビューで 5 件訂正した経緯も含めて開示する。
