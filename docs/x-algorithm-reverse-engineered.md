...
> **TL;DR**: 124人規模の本アカウントでは、自発投稿の imp 上限は約180、リプの outlier は2,000+に届いた。この差から、他者のオーディエンスに届くリプを動線とし、プロフィールと自発投稿を受皿にするハイブリッド型が有効だと推測した。これは1アカウントの観測であり、X全体のアルゴリズムを確定するものではない。
...
2. **リプの imp 分布は二極化**: 大半が 5-30 imp、稀に 500-2,000 imp の外れ値が出た
3. **16週の分析で、リプ数がフォロワー週次増減と最も明確に連動した**（自発投稿の頻度・平均いいね・平均 imp には同じ傾向が見られなかった）
...
左が「自発投稿のみに頼るモデル（Cold Start）」、右が本アカウントで採用した「Borrowed Audience + 受皿ハイブリッド」。
...
124人規模の本アカウントでは、自発投稿だけで認知を広げるのは困難だった:
...
本観測では、自発投稿だけでは 180 imp 前後を超えにくかった。一方で craft は、動線から来た読者を離脱させない受皿として機能したため、不要だったわけではない。
