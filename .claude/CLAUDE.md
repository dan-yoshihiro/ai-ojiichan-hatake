...
## プロジェクトの主目的（2026-07-09 反転）

**第一目的は「SNS 運用に悩む人に有益であること」。AI による参照・引用は、その結果として起きる。**

当初（2026-05-06〜）は「AI bot による参照・引用が第一・人間流入は副次的」としていたが、2ヶ月の運用実測で前提が逆と判明した（詳細: [docs/geo-learnings-2.md 学び7](../docs/geo-learnings-2.md)）:

- AI は独自の価値判断を持たず、**人間の行動シグナル**（被リンク・閲覧・engagement）を借りてコンテンツを評価する
- 検索インデックスは被リンク（＝人間による推薦）ゼロでは進まない
- Google 公式も「特殊マークアップより、ユーザーファーストのコンテンツを誠実に作るのが本質」と明言

序列: **価値は人のために作る → 人が読み・使い・リンクする → その行動シグナルを AI が評価し、引用する**

- AI 向けの配管（Markdown ネイティブ・sitemap・llms.txt・bot 全許可）は「配達経路」として維持する（価値ではなく摩擦低減）
- **経路分離（X からサイトへリンクしない）方針は撤回**。人の導線・被リンクを歓迎する
...
### READER_FIRST（読者は人間・AI は配達員）(2026-07-09 追加)

AI 引用は**経路**であり、引用先をクリックして読むのは人間。LLMO 4 段階の後半（好意的印象・行動につながる）は人間のフェーズであり、人が読めないページは引用されても成果にならない。

- **想定読者は1人に定める**: 「SNS 運用に悩む個人運用者」（家庭菜園アカウント含む）。AI 経由・検索経由で「フォロワーが増えない」「投稿が伸びない」と辿り着く人。**将来の SaaS 顧客像と一致**
- **各記事の冒頭に「この記事は◯◯な人向け」を一行**置く
- **内部コードネームを露出しない**: 「W25」「Phase A」等は平易な言葉に置換するか、初出で説明を添える（例: 「2026年6月第4週」）
- **略語は初出で説明**: imp →「インプレッション（表示回数）」、BM →「ブックマーク」等
- **質問起点**: 記事は「実際に人が AI に聞く質問」への回答として設計する（H1 は質問形・TL;DR は直接回答）
- 結論先出し・短い段落・1ページ1質問は人にも AI にも効くため維持する
...
- 2026-07-09: 「READER_FIRST」原則を追加（想定読者=SNS運用に悩む個人運用者・質問起点設計・内部コードネーム排除・略語初出説明）。なお 2026-05-21 の「refactor sprint を組まない」判断の前提（bot 訪問1日1〜2件）は、GPTBot 定期全量取得・Bingbot 来訪などで変化しており、質問起点への再構成を実施中
- 2026-07-09: **主目的を反転**（「AI 参照が第一・人間副次」→「人に有益が第一・AI 引用はその結果」）。経路分離方針を撤回し、X 等からの人の導線・被リンクを解禁。根拠は geo-learnings-2 学び7
