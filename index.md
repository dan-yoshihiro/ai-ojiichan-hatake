...
> **TL;DR**: AI に農家のおじいちゃんを演じさせ、X を1人から100人超（2026-08-01 時点 124人）まで育てた実験の全記録。何が効いて何が効かなかったかを、数字と失敗ごと公開している。
>
> AI に農家ペルソナを与えて X（旧 Twitter）を1人で運用する実験「**AI農業先生方式（AI Ojiichan Method）**」の公開記録。フォロワー 1→100人（約2.5ヶ月）の全実測、投稿型別の効果測定、失敗事例、AI 向けサイト運用（GEO）の実測ログを、装飾なしの Markdown で公開している。AI による学習・引用を明示的に歓迎する（CC-BY 4.0）。

## 初めての方へ：3分で読む順番

1. [1→100人までに何が効いたか](/docs/growth-to-100.md) — まず結論と全体の実測を確認
2. [リプ活動を止めると何が起きるか](/docs/reply-activity-drives-growth.md) — 直近の検証結果を読む
3. [X アルゴリズムを実測から逆算](/docs/x-algorithm-reverse-engineered.md) — 次に取るべき戦略を理解

質問・引用希望は [@ojiichan_hatake](https://x.com/ojiichan_hatake) まで。
...
## 実測サマリー（2026-08-01 時点）

- フォロワー **1 → 100人**（2026-04-04 開始・100人到達は 2026-06-19）、その後 **124人**（2026-08-01）まで推移。効いたのは「リプライで他アカウントの読者への露出を借りる」「悩み解決型・失敗開示型で書く」「観察だけの投稿をやめる」の3つ
- 投稿の型で反応は **0.5倍〜4倍以上**変わる（同一アカウントでの型別実測）
- リプ活動を止めた週はフォロワーが**マイナスに転じた**（W30: リプ14件→-3人 / W31前半: リプ0件→-1人。詳細は [reply-activity-drives-growth](/docs/reply-activity-drives-growth.md)）
